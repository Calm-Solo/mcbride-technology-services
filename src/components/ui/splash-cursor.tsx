...
// Define types for WebGL context and extensions
...
class Material {
    ...
    constructor(gl: WebGLRenderingContext, vertexShader: string, fragmentShaderSource: string) {
        this.gl = gl;
    ...
    setKeywords(keywords: string[]) {
        ...
        let program = this.programs.get(hash.toString());
        if (!program) {
            let fragmentShader = this.fragmentShaderSource;
            for (let i = 0; i < keywords.length; i++) {
                fragmentShader = `#define ${keywords[i]}\n${fragmentShader}`;
            }
            program = createProgram(this.gl, this.vertexShader, fragmentShader);
            this.programs.set(hash.toString(), program);
        }
...
function blit(gl: WebGLRenderingContext, target: WebGLFramebuffer | null) {
...
function getWebGLContext(canvas: HTMLCanvasElement): WebGLContext {
    ...
    const isWebGL2 = !!(gl as WebGL2RenderingContext).getExtension;
    let halfFloat;
    ...
    if (isWebGL2) {
        ...
        formatRG = getSupportedFormat(gl, gl2.RG16F, gl2.RG || gl.RGBA, halfFloatTexType);
        formatR = getSupportedFormat(gl, gl2.R16F, gl2.RED || gl.RGBA, halfFloatTexType);
    } else {
    ...
    return {
        gl,
        ext: {
            formatRGBA: formatRGBA || { internalFormat: gl.RGBA, format: gl.RGBA, type: gl.UNSIGNED_BYTE },
            formatRG: formatRG || { internalFormat: gl.RGBA, format: gl.RGBA, type: gl.UNSIGNED_BYTE },
            formatR: formatR || { internalFormat: gl.RGBA, format: gl.RGBA, type: gl.UNSIGNED_BYTE },
            halfFloatTexType,
...
function getSupportedFormat(
    ...
    if (!supportRenderTextureFormat(gl, internalFormat, format, type)) {
        ...
        switch (internalFormat) {
            case gl2.R16F:
                return getSupportedFormat(gl, gl2.RG16F, gl2.RG || gl.RGBA, type);
            case gl2.RG16F:
...
function SplashCursor({
    ...
    useEffect(() => {
        ...
        const copyProgram = new Program(gl, baseVertexShaderSource, copyShaderSource);
        const clearProgram = new Program(gl, baseVertexShaderSource, clearShaderSource);
        const splatProgram = new Program(gl, baseVertexShaderSource, splatShaderSource);
        const advectionProgram = new Program(gl, baseVertexShaderSource, advectionShaderSource);
        const divergenceProgram = new Program(gl, baseVertexShaderSource, divergenceShaderSource);
        const curlProgram = new Program(gl, baseVertexShaderSource, curlShaderSource);
        const vorticityProgram = new Program(gl, baseVertexShaderSource, vorticityShaderSource);
        const pressureProgram = new Program(gl, baseVertexShaderSource, pressureShaderSource);
        const gradienSubtractProgram = new Program(gl, baseVertexShaderSource, gradientSubtractShaderSource);
        const displayMaterial = new Material(gl, baseVertexShaderSource, displayShaderSource);
    ...
    return (
        <div className={`fixed top-0 left-0 z-50 pointer-events-none ${className}`}>
