...
export default function ServiceCard({ title, description, features, className = '' }: ServiceCardProps) {
    return (
        <div
            className={`card min-w-sm max-w-sm flex flex-wrap h-full bg-st_dark/50 p-6 rounded-lg border border-st_light/20 hover:border-primary/50 transition-all ${className}`}>
